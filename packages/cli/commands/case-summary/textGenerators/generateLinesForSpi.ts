...
const generateLinesForSpi = async (s3Client: S3Client, s3Path: string, addLine: AddLine): Promise<void> => {
  ...
  const timestamp = `${s3PathParts[0]}-${s3PathParts[1]}-${s3PathParts[2]} ${s3PathParts[3]}:${s3PathParts[4]}`

  addLine("==================================================================")
  addLine(`SPI OUT: Court hearing outcome\t${timestamp}`)
  addLine("==================================================================")
